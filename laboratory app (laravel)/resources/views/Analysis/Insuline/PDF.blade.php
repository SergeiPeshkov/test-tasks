...
<html>
    ...
    <head>
        ...
        <title>{{$Insuline->analiseType}}</title>
        <style>
    ...
    <br>
        ...
        <table class="table2">
            <tr>
                <td>ФИО:{{$Insuline->patient_lastname}}   {{$Insuline->patient_name}}  {{$Insuline->patient_patronymic}}</td>              
                <td>№ договора: {{$Insuline->contract_number}}</td>                         
              </tr>  

            <tr>
                @if($Insuline->age)
                <td>Возраст пациента: {{$Insuline->age}} </td>    
                @endif
               
                <td>Дата обращения: {{date('d.m.yy', strtotime($Insuline->dateVisit))}} </td>
            </tr>
           <tr>
            @if($Insuline->sex)
            <td>Пол пациента: {{$Insuline->sex}} </td>    
            @endif
           </tr>
        
        </table>
      
        <h3 style="text-align:center; font-size:24px;">{{$Insuline->analiseType}}</h3>
        ...
        @if($Insuline->insuline)
            <tr>
                <td>Инсулин  </td>           
                <td> <i>{{$Insuline->insuline}} </i></td>
                <td>0,7-9,0 мкЕ/мл -нет диабета<br>
                    0,7-25 мкЕ/мл - диабет II типа  </td>
            </tr>
        @endif

        @if($Insuline->indexHoma)
            <tr>
                <td>Индекс HOMA    </td>           
                <td> <i>{{$Insuline->indexHoma}}  </i></td>
                <td>до 3,0 </td>
            </tr>
        @endif
        ...
        <table class="table5"> 
            <tr>                     
               <td width="50%">Анализ выполнил: {{$Insuline->laborant}} </td> 
               <td rowspan="3" width="5%"><img src="{{$path['info']}}"> </td>
               <td  rowspan="3">                     
                   <i>Анализ выполнен на автоматическом ИХЛ анализаторе</i>              
                        <strong>{{$Insuline->analyser}}</strong>
               </td>
           </tr>    
           <tr>
                   <td width="30%">Дата: {{date('d.m.yy', strtotime($Insuline->dateResult))}} </td>
           </tr>  
        </table>
