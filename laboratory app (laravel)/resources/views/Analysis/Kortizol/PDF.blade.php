...
<html>
    ...
    <head>
        ...
        <title>{{$Kortizol->analiseType}}</title>
        <style>
    ...
    <br>
        ...
        <table class="table2">
            <tr>
                <td>ФИО:{{$Kortizol->patient_lastname}}  {{$Kortizol->patient_name}}  {{$Kortizol->patient_patronymic}}</td>              
                <td>№ договора: {{$Kortizol->contract_number}}</td>                         
              </tr>  

            <tr>
                @if($Kortizol->age)
                <td>Возраст пациента: {{$Kortizol->age}} </td>    
                @endif
               
                <td>Дата обращения: {{date('d.m.yy', strtotime($Kortizol->dateVisit))}} </td>
            </tr>
           <tr>
            @if($Kortizol->sex)
            <td>Пол пациента: {{$Kortizol->sex}} </td>    
            @endif
           </tr>
        
        </table>
      
        <h3 style="text-align:center; font-size:24px;">{{$Kortizol->analiseType}}</h3>
        ...
        @if($Kortizol->kortizol)
            <tr>
                <td>Кортизол    </td>           
                <td> <i>{{$Kortizol->kortizol}}  </i></td>
                <td>150-660 нмоль/л  </td>
            </tr>
        @endif
        ...
        <table class="table5"> 
            <tr>                     
               <td width="50%">Анализ выполнил: {{$Kortizol->laborant}} </td> 
               <td rowspan="3" width="5%"><img src="{{$path['info']}}"> </td>
               <td  rowspan="3">                     
                   <i>Анализ выполнен на автоматическом ИХЛ анализаторе</i>              
                        <strong>{{$Kortizol->analyser}}</strong>
               </td>
           </tr>    
           <tr>
                   <td width="30%">Дата: {{date('d.m.yy', strtotime($Kortizol->dateResult))}} </td>
           </tr>  
        </table>
