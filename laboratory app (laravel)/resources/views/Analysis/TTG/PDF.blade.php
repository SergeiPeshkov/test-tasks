...
<html>
    ...
    <br>
        ...
        <table class="table2">
            <tr>
                <td>ФИО: {{$TTG->patient_lastname}} {{$TTG->patient_name}}  {{$TTG->patient_patronymic}}</td>              
                <td>№ договора: {{$TTG->contract_number}}</td>                         
              </tr>  
            <tr>
                       
              <td>Дата обращения: {{date('d.m.yy', strtotime($TTG->dateVisit))}} </td>
            </tr>
           
        
        </table>
        <br>
        <h3 style="text-align:center; font-size:24px;">{{$TTG->analiseType}}</h3>
        ...
        <tr>
            ...
            <td> <i>{{$TTG->ttg}} </i></td>
            <td>0-1,22 МЕ/л -нет заболеваний щитовидной железы<br>
                0-1,58 МЕ/л - при заболеваниях щитовидной железы без болезни Грейвса </td>
        </tr>
   ...
   <table class="table5"> 
            <tr>                     
               <td width="50%">Анализ выполнил: {{$TTG->laborant}} </td> 
               <td rowspan="3" width="5%"><img src="{{$path['info']}}"> </td>
               <td  rowspan="3">                     
                   <i>Анализ выполнен на автоматическом ИХЛ анализаторе</i>              
                        <strong>{{$TTG->analyser}}</strong>
               </td>
           </tr>    
           <tr>
                   <td width="30%">Дата: {{date('d.m.yy', strtotime($TTG->dateResult))}} </td>
           </tr>  
        </table>
