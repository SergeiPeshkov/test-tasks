...
<html>
    ...
    <head>
        ...
        <title>{{$Hpylori->analiseType}}</title>
        <style>
    ...
    <br>
        ...
        <table class="table2">
            <tr>
                <td>ФИО: {{$Hpylori->patient_lastname}} {{$Hpylori->patient_name}}  {{$Hpylori->patient_patronymic}}</td>              
                <td>№ договора: {{$Hpylori->contract_number}}</td>                         
              </tr>  

            <tr>
                @if($Hpylori->age)
                <td>Возраст пациента: {{$Hpylori->age}} </td>    
                @endif
               
                <td>Дата обращения: {{date('d.m.yy', strtotime($Hpylori->dateVisit))}} </td>
            </tr>
           <tr>
            @if($Hpylori->sex)
            <td>Пол пациента: {{$Hpylori->sex}} </td>    
            @endif
           </tr>
        
        </table>
      
        <h3 style="text-align:center; font-size:24px;">{{$Hpylori->analiseType}}</h3>
        ...
        <table class="table5"> 
            <tr>                     
               <td width="50%">Анализ выполнил: {{$Hpylori->laborant}} </td> 
               <td rowspan="3" width="5%"><img src="{{$path['info']}}"> </td>
               <td  rowspan="3">                     
                   <i>Анализ выполнен на автоматическом ИФА анализаторе</i>              
                        <strong>{{$Hpylori->analyser}}</strong>
               </td>
           </tr>    
           <tr>
                   <td width="30%">Дата: {{date('d.m.yy', strtotime($Hpylori->dateResult))}} </td>
           </tr>  
        </table>
