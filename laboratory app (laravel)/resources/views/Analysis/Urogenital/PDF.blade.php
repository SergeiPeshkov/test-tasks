...
<html>
    ...
    <head>
        ...
        <title>{{$Urogenital->analiseType}}</title>
        <style>
    ...
    <br>
        ...
        <table class="table2">
            <tr>
                <td>ФИО: {{$Urogenital->patient_lastname}} {{$Urogenital->patient_name}}  {{$Urogenital->patient_patronymic}}</td>              
                <td>№ договора: {{$Urogenital->contract_number}}</td>                         
              </tr>  
            <tr>
                       
              <td>Дата обращения: {{date('d.m.yy', strtotime($Urogenital->dateVisit))}} </td>
            </tr>
           
        
        </table>
        <br>
        <h3 style="text-align:center; font-size:24px;">{{$Urogenital->analiseType}}</h3>
        ...
        @if($Urogenital->hlamidii_ygm)
            <tr>
                <td>Хламидии YgM </td>           
                <td> <i>{{$Urogenital->hlamidii_ygm}} </i></td>
                <td>Отрицательный </td>
            </tr>
        @endif

        @if($Urogenital->hlamidii_ygg)
            <tr>
                <td>Хламидии YgG </td>           
                <td> <i>{{$Urogenital->hlamidii_ygg}} </i></td>
                <td>Отрицательный</td>
            </tr>
        @endif

        @if($Urogenital->ureplasma)
            <tr>
                <td>Уреплазма IgG</td>           
                <td> <i>{{$Urogenital->ureplasma}} </i></td>
                <td>Отрицательный</td>
            </tr>
        @endif

        @if($Urogenital->mikoplasma)
            <tr>
                <td>Микоплазма IgG</td>           
                <td> <i>{{$Urogenital->mikoplasma}} </i></td>
                <td>Отрицательный </td>
            </tr>
       @endif
        ...
        <table class="table5"> 
            <tr>                     
               <td width="50%">Анализ выполнил: {{$Urogenital->laborant}} </td> 
               <td rowspan="3" width="5%"><img src="{{$path['info']}}"> </td>
               <td  rowspan="3">                     
                   <i>Анализ выполнен на автоматическом коагулометре</i>              
                        <strong>{{$Urogenital->analyser}}</strong>
               </td>
           </tr>    
           <tr>
                   <td width="30%">Дата: {{date('d.m.yy', strtotime($Urogenital->dateResult))}} </td>
           </tr>  
        </table>
