...
<html>
    ...
    <br>
        ...
        <table class="table2">
            <tr>
                <td>ФИО:{{$Toxo->patient_lastname}}   {{$Toxo->patient_name}}  {{$Toxo->patient_patronymic}}</td>              
                <td>№ договора: {{$Toxo->contract_number}}</td>                         
              </tr>  

            <tr>
                @if($Toxo->age)
                <td>Возраст пациента: {{$Toxo->age}} </td>    
                @endif
               
                <td>Дата обращения: {{date('d.m.yy', strtotime($Toxo->dateVisit))}} </td>
            </tr>
            
            @if($Toxo->sex)
                <tr>         
                    <td>Пол пациента: {{$Toxo->sex}} </td>               
                </tr>
           @endif
        </table>
      
        <h3 style="text-align:center; font-size:24px;">{{$Toxo->analiseType}}</h3>
       ...
       @if($Toxo->ygm)
        ...
        <br>
         ...
         <table class="table5"> 
            <tr>                     
               <td width="50%">Анализ выполнил: {{$Toxo->laborant}} </td> 
               <td rowspan="3" width="5%"><img src="{{$path['info']}}"> </td>
               <td  rowspan="3">                     
                   <i>Анализ выполнен на автоматическом ИФА анализаторе</i>              
                        <strong>{{$Toxo->analyser}}</strong>
               </td>
           </tr>    
           <tr>
                   <td width="30%">Дата: {{date('d.m.yy', strtotime($Toxo->dateResult))}} </td>
           </tr>  
        </table>
        <br>      
