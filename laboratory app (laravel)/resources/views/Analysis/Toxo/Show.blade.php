...
<div class="container-new-contract">
	...
	<form class="form-new-contract" action="{{route('ToxoPDF', ['id'=>$result->id])}}" method="POST">
		@csrf
		

<h3>Инфекции (токсоплазма) |
...
<div class="input-group form-row">         
  ...
  @if($result->ygm)
    <tr>
      ...
      <td width="15%">    
      <p class="fieldOutputAnalis">{{$result->ygm}}</p> 
      </td>
  ...
  @if($result->ygg)
    <tr>
      ...
      <td width="15%">    
      <p class="fieldOutputAnalis">{{$result->ygg}}</p> 
      </td>
