...
<html>
    ...
    <br>
        ...
        <table class="table2">
            <tr>
                <td>ФИО:  {{$Infekcii->patient_lastname}} {{$Infekcii->patient_name}} {{$Infekcii->patient_patronymic}}</td>              
                <td>№ договора: {{$Infekcii->contract_number}}</td>                         
              </tr>  
            <tr>
             
              <td>Возраст пациента: {{$Infekcii->age}}</td>  
              <td>Дата обращения: {{date('d.m.yy', strtotime($Infekcii->dateVisit))}} </td>
            </tr>
        ...
        <h3 style="text-align:center; font-size:24px;">Инфекции (ВПГ, ЦМВ)</h3>
      ...
      <table class="table5"> 
            <tr>                     
               <td width="50%">Анализ выполнил: {{$Infekcii->laborant}} </td> 
               <td rowspan="3" width="5%"><img src="{{$path['info']}}"> </td>
               <td  rowspan="3">                     
                   <i>Анализ выполнен на автоматическом ИФА анализаторе</i>              
                        <strong>{{$Infekcii->analyser}}</strong>
               </td>
           </tr>    
           <tr>
                   <td width="30%">Дата: {{date('d.m.yy', strtotime($Infekcii->dateResult))}} </td>
           </tr>  
        </table>
