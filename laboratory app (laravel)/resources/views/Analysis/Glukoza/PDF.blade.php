...
<html>
    ...
    <head>
        ...
        <title>{{$glukoza->analiseType}}</title>
        <style>
    ...
    <br>
        ...
        <table class="table2">
            <tr>
                <td>ФИО: {{$glukoza->patient_lastname}} {{$glukoza->patient_name}}  {{$glukoza->patient_patronymic}}</td>              
                <td>№ договора: {{$glukoza->contract_number}}</td>                         
              </tr>  

            <tr>
                @if($glukoza->age)
                <td>Возраст пациента: {{$glukoza->age}} </td>    
                @endif
               
                <td>Дата обращения: {{date('d.m.yy', strtotime($glukoza->dateVisit))}} </td>
            </tr>
           <tr>
            @if($glukoza->sex)
            <td>Пол пациента: {{$glukoza->sex}} </td>    
            @endif
           </tr>
        
        </table>
      
        <h3 style="text-align:center; font-size:24px;">{{$glukoza->analiseType}}</h3>
        ...
        <tr>
            <td>Глюкоза </td>           
            <td> <i>{{$glukoza->glukoza}} </i></td>
            <td>3,33-5,55  	ммоль/л  </td>
        </tr>
        ...
        <table class="table5"> 
            <tr>                     
               <td width="50%">Анализ выполнил: {{$glukoza->laborant}} </td> 
               <td rowspan="3" width="5%"><img src="{{$path['info']}}"> </td>
               <td  rowspan="3">                     
                   <i>Анализ выполнен на автоматическом ИФА анализаторе</i>              
                        <strong>{{$glukoza->analyser}}</strong>
               </td>
           </tr>    
           <tr>
                   <td width="30%">Дата: {{date('d.m.yy', strtotime($glukoza->dateResult))}} </td>
           </tr>  
        </table>
