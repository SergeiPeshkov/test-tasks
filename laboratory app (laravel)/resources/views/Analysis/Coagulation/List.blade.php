	...
	<form action="{{route('analisysList')}}" method="GET">
        ...
        <div class="row" style="margin-left: -15px;">
            ...
            <div class="col-md-4">
                ...
                <option value="{{$slug}}">Показатели системы свертывания крови </option>   
                <option>--------------------------------------</option>  
...
<!--
                            @foreach($patientList as $PatientItem)  
                                @if($resultItem->patient_id == $PatientItem->id)
                                <p style="line-height: 1.5;"> <a href="{{ route('patientShow', ['id'=>$PatientItem->id]) }}">   {{$PatientItem->name}} {{$PatientItem->lastname}} {{$PatientItem->patronymic}} </a></p> 
                                @endif
                            @endforeach  
                        -->
                          {{$resultItem->patient_lastname}}    {{$resultItem->patient_name}}  {{$resultItem->patient_patronymic}} 
                            ...
                            <td>                                
                            <p style="line-height: 1.5;"> <a href="{{ route('CoagulationShow', ['id'=>$resultItem->id]) }}">  {{$resultItem->analiseType}} </a></p> 
                            </td>
