...
<html>
    ...
    <head>
        ...
        <title>{{$Antifosfolipid->analiseType}}</title>
        <style>
    ...
    <br>
        ...
        <table class="table2">
            <tr>
                <td>ФИО: {{$Antifosfolipid->patient_lastname}}  {{$Antifosfolipid->patient_name}}  {{$Antifosfolipid->patient_patronymic}}</td>              
                <td>№ договора: {{$Antifosfolipid->contract_number}}</td>                         
            </tr>  

            <tr>
                @if($Antifosfolipid->age)
                <td>Возраст пациента: {{$Antifosfolipid->age}} </td>    
                @endif
               
                <td>Дата обращения: {{date('d.m.yy', strtotime($Antifosfolipid->dateVisit))}} </td>
            </tr>
           <tr>
            @if($Antifosfolipid->sex)
            <td>Пол пациента: {{$Antifosfolipid->sex}} </td>    
            @endif
           </tr>
        
        </table>
      
        <h3 style="text-align:center; font-size:24px;">{{$Antifosfolipid->analiseType}}</h3>
        ...
        @if($Antifosfolipid->lgg)        
            <tr>
                <td>Антифосфолипидный скрининг LgG  </td>           
                <td> <i>{{$Antifosfolipid->lgg}} </i></td>
                <td>< 10 отрицательный<br>
                    >= 10 положительный   </td>
            </tr>
        @endif

        
        @if($Antifosfolipid->lgm)
        <tr>
            <td>Антифосфолипидный скрининг LgM   </td>           
            <td> <i>{{$Antifosfolipid->lgm}}  </i></td>
            <td>< 10 отрицательный<br>
                >= 10 положительный </td>
        </tr>
        @endif
        ...
        <table class="table5"> 
            <tr>                     
               <td width="50%">Анализ выполнил: {{$Antifosfolipid->laborant}} </td> 
               <td rowspan="3" width="5%"><img src="{{$path['info']}}"> </td>
               <td  rowspan="3">                     
                   <i>Анализ выполнен на автоматическом анализаторе</i>              
                        <strong>{{$Antifosfolipid->analyser}}</strong>
               </td>
           </tr>    
           <tr>
                   <td width="30%">Дата: {{date('d.m.yy', strtotime($Antifosfolipid->dateResult))}} </td>
           </tr>  
        </table>
        <br>        
