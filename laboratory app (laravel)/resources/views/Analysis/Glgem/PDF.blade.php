...
<html>
    ...
    <head>
        ...
        <title>Гликозилированный гемоглобин</title>
        <style>
    ...
    <br>
        ...
        <table class="table2">
            <tr>
                <td>ФИО:  {{$Glgem->patient_lastname}} {{$Glgem->patient_name}} {{$Glgem->patient_patronymic}}</td>              
                <td>№ договора: {{$Glgem->contract_number}}</td>                         
              </tr>  
            <tr>
             
              <td>Возраст пациента: {{$Glgem->age}}</td>  
              <td>Дата обращения: {{date('d.m.yy', strtotime($Glgem->dateVisit))}} </td>
            </tr>
           
        
        </table>
        <br>
        <h3 style="text-align:center; font-size:24px;">Гликозилированный гемоглобин</h3>
          <table class="table3" border="1">
        <tr>
            <th>Результат:</th>
            <th>HbA 1c % Референсные уровни</th>            
        </tr>
        <tr>           
            <td> <i>{{$Glgem->result}}% </i></td>
            <td>
                < 6 нет диабета <br>
                > 7 норма при диабете <br> 
                > 8 требуется коррекция
            </td>
        </tr>
        ...
        <table class="table5"> 
            <tr>                     
               <td width="50%">Анализ выполнил: {{$Glgem->laborant}} </td> 
               <td rowspan="3" width="5%"><img src="{{$path['info']}}"> </td>
               <td  rowspan="3">                     
                   <i>Анализ выполнен на полуавтоматическом биохимическом анализаторе</i>              
                        <strong>{{$Glgem->analyser}}</strong>
               </td>
           </tr>    
           <tr>
                   <td width="30%">Дата: {{date('d.m.yy', strtotime($Glgem->dateResult))}} </td>
           </tr>  
        </table>
